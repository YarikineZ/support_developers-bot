...
def start(update, context):
    ...
    context.bot.send_message(
        ...
        text=f"""
📞 К боту техподдержки Developers присоединился новый пользователь #{user_info.id} {user_info.first_name} (@{user_info.username}).\n Данные пользователя: \n Имя: {user_info.first_name} , \n Фамилия: {user_info.last_name} , \n Username: @{user_info.username} , \n Бот? {user_info.is_bot}.
        """,
    )
...
def forward_to_chat(update, context):
    ...
    if not forwarded.forward_from:
        context.bot.send_message(
            ...
            text=f'{update.message.from_user.id} \nПользователь {update.message.from_user.first_name}(@{update.message.from_user.username}) не разрешает пересылать свои сообщения. {REPLY_TO_THIS_MESSAGE}'
        )
